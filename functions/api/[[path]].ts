...
import authDebug from '../../api/auth-debug';
...
export const onRequest: PagesFunction = async (context) => {
  ...
  if (!handler) {
    ...
    // a non-API path must be served by ASSETS (which applies _redirects and
    // the /* /index.html 200 SPA fallback) instead of a 404 JSON.
    try {
      return (context.env as any).ASSETS.fetch(request);
    } catch (e: any) {
      return new Response(JSON.stringify({ error: e?.message || 'static fallback failed', path: name }), {
        status: 500,
