...
export default function CheckpointView() {
  ...
  const handleAnswer = (answer: string) => {
    ...
    // Record the chosen option index for server-side grading.
    const idx = currentQuestion?.options.indexOf(answer) ?? -1;
  ...
  const finishCheckpoint = async () => {
    ...
    if (user && id) {
      setSubmitting(true);
      // Server-side grading (N4): send the selected answer indices, not the
      // score — the server computes pass/fail from the seeded question bank.
      await submitCheckpoint(user.id, id, selectedAnswers.length ? selectedAnswers : answers.map((ok, i) => (ok ? i : -1)));
      setSubmitting(false);
    }
  };
