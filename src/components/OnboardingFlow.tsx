...
export default function OnboardingFlow({ onComplete }: { onComplete: () => void }) {
  ...
  const finish = async (destination: string) => {
    ...
    if (user) {
      try {
        // Server resolves the internal UUID from the verified JWT; the Clerk id
        // in `user.id` is not a key the server reads. Flag write only.
        await dbProxy('upsert-profile', { onboarding_completed: true });
      } catch (e) {
