...
export function useAdminFeedback(): FeedbackApi {
  ...
  const notify = React.useCallback(
    (tone: Tone, title: string, detail?: string) => {
      ...
      // Dedupe: same-title toast replaces the old one instead of stacking —
      // double-firing the same action (save, validate) produced duplicate
      // toasts on mobile ("Provider belum tersimpan" x2, one faded).
      setItems(prev => {
        const without = prev.filter(i => i.title !== title);
        return [...without.slice(-2), { id, tone, title, detail }];
      });
...
export function AdminFeedbackStack({ feedback }: { feedback: FeedbackApi }) {
  ...
  return (
    <div
      className="fixed inset-x-3 top-3 z-50 flex flex-col gap-2 sm:inset-x-auto sm:right-5 sm:top-5 sm:w-96"
      role="status"
      aria-live="polite"
    >
      {items.map(item => {
