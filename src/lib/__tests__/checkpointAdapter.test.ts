...
describe('checkpointAdapter', () => {
  it('resolves legacy top-level checkpoint lessons used by CheckpointView routes', () => {
    const resolved = resolveCheckpointLesson(testLessons, 'a1-checkpoint-1');

    expect(resolved?.lesson.id).toBe('a1-checkpoint-1');
    expect(resolved?.checkpoint.title).toMatch(/Review/i);
  ...
  it('resolves current nested checkpoints where the route id lives in lesson.checkpoint.id', () => {
    const resolved = resolveCheckpointLesson(testLessons, 'a2-checkpoint-1');
  ...
  it('resolves every authored checkpoint plus all derivable level-closing ones', () => {
    const resolvable = listResolvableCheckpointIds(testLessons);
    ...
    for (const id of derivableCheckpointIds) {
      expect(resolvable, id).toContain(id);
    }
  });

  it('rebuilds level-closing checkpoints from the lessons they review', () => {
    const resolved = resolveCheckpointLesson(testLessons, 'a1-checkpoint-4');
    ...
    // Review spans the level, not just its opening lessons.
    expect(resolved?.checkpoint.reviewLessons).toContain('a1-1');
    expect(resolved?.checkpoint.reviewLessons.some((id) => Number(id.split('-')[1]) > 5)).toBe(true);
  });
  ...
  it('only ever reuses real authored questions from the lessons it names', () => {
    ...
    const sourceQuestions = new Set(
      (resolved?.checkpoint.reviewLessons || [])
        .map((id) => testLessons.find((lesson) => lesson.id === id))
  ...
  it('keeps derived questions scoreable: CheckpointView compares option strings', () => {
    for (const id of derivableCheckpointIds) {
      const resolved = resolveCheckpointLesson(testLessons, id);
      ...
      const rederived = resolveCheckpointLesson(testLessons, id, [...courseIndex]);
