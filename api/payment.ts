...
export default async function handler(req: ApiRequest, res: ApiResponse) {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      // Idempotency guard (audit VULN-02: 5 concurrent creates → 5 distinct
      // charges at the gateway, Rp245K exposure). One user, one pending order.
      // If an active pending order exists, return it instead of charging again.
      const { data: existingOrder, error: existingError } = await getDb()
        .from('orders')
        .select('*')
        .eq('user_id', userId)
        .eq('status', 'pending')
      ...
      if (existingError) {
        console.error('[payment/create] Pending order lookup error:', existingError);
        return res.status(500).json({ error: 'Failed to check existing order' });
      }

      if (existingOrder) {
        // Pending orders from this user older than 24h are stale (gateway
        // charge already expired) — treat as absent, let a fresh order be made.
        const createdAt = existingOrder.created_at ? new Date(existingOrder.created_at).getTime() : 0;
        const stale = !createdAt || Date.now() - createdAt > 24 * 60 * 60 * 1000;
        if (stale) {
          await getDb().from('orders').update({ status: 'expired' }).eq('id', existingOrder.id);
          console.log('[payment/create] Stale pending order expired:', existingOrder.id);
        } else {
          console.log('[payment/create] Existing pending order, reusing:', existingOrder.id);
          return res.json({
            url: existingOrder.payment_url || '',
            invoice_id: existingOrder.id,
      ...
      if (error) {
        // Unique partial index idx_orders_one_pending_per_user: a concurrent
        // create already inserted a pending order for this user (race).
        // Reuse theirs instead of failing — the gateway charge we just made is
        // harmless (expires unpaid), the response stays a valid checkout.
        const isDup = /idx_orders_one_pending_per_user|duplicate key/i.test(
          typeof error.message === 'string' ? error.message : String(error)
        );
        if (isDup) {
          console.warn('[payment/create] Concurrent create detected, reusing pending order for user:', userId);
          const { data: concurrentOrder } = await getDb()
            .from('orders')
            .select('*')
            .eq('user_id', userId)
            .eq('status', 'pending')
          ...
          if (concurrentOrder) {
            return res.json({
              url: concurrentOrder.payment_url || '',
              invoice_id: concurrentOrder.id,
              amount: concurrentOrder.amount,
              expires_at: concurrentOrder.expires_at || null,
              reused: true,
            });
          }
        }
